...
const pageExamples = [
  {
    slug: 'login',
    name: 'Login Page',
    demos: [{ name: 'Basic Login', href: '/examples/login' }],
  },
  {
    slug: 'dashboard',
    name: 'Dashboard',
    demos: [{ name: 'Overview Dashboard', href: '/examples/dashboard' }],
  },
  {
    slug: 'form',
    name: 'Form Page',
    demos: [{ name: 'Contact Form', href: '/examples/form' }],
  },
  {
    slug: 'landing',
    name: 'Landing Page',
    demos: [{ name: 'Hero Landing', href: '/examples/landing' }],
  },
  {
    slug: 'settings',
    name: 'Settings Page',
    demos: [{ name: 'Account Settings', href: '/examples/settings' }],
  },
  {
    slug: 'profile',
    name: 'Profile Page',
    demos: [{ name: 'User Profile', href: '/examples/profile' }],
  },
]
...
function PageSubSection({
  section,
  pathname,
}: {
  section: (typeof pageExamples)[number]
  pathname: string
}) {
  const hasActive = section.demos.some((d) => pathname === d.href)
  const [open, setOpen] = useState(hasActive)
  ...
  return (
    <div>
      ...
      {open && (
        <div className="ml-3 mt-0.5 flex flex-col gap-0.5 border-l border-vui-border pl-3">
...
export function DocsSidebar() {
  ...
  const examplesActive = pageExamples.some((p) =>
    p.demos.some((d) => pathname === d.href)
  )

  const [componentsOpen, setComponentsOpen] = useState(componentsActive || !examplesActive)
    ...
    <aside className="sticky top-20 flex h-[calc(100vh-5rem)] w-56 flex-shrink-0 flex-col">
      <nav className="flex flex-col gap-1 overflow-y-auto pr-1">
        ...
        <div className="mt-2">
          ...
          {examplesOpen && (
            <div className="mt-0.5 flex flex-col gap-0.5">
              {pageExamples.map((section) => (
                <PageSubSection key={section.slug} section={section} pathname={pathname} />
              ))}
            </div>
          )}
        </div>
