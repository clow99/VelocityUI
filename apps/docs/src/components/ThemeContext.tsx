...
import { THEMES, DENSITIES, type ThemeName, type DensityName } from '@/lib/themes'
export { THEMES, DENSITIES, type ThemeName, type DensityName } from '@/lib/themes'
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const storedTheme = readStorage<string>(STORAGE_KEY_THEME, 'default')
    const storedDensity = readStorage<string>(STORAGE_KEY_DENSITY, 'comfortable')
    setThemeState(isThemeName(storedTheme) ? storedTheme : 'default')
